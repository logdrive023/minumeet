...
export async function POST() {
  try {
    ...
    const storageRlsSQL = `
      -- Permitir que usuários autenticados façam upload de arquivos para suas próprias pastas
      BEGIN;
      ...
      -- Permitir acesso público para leitura de todos os arquivos no bucket avatars
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_policies 
